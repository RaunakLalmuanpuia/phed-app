<template>
    <q-page class="container" padding>
        ...
        <div class="flex items-center justify-between q-pa-md bg-white">
            <div>
                <div class="stitle">Remuneration Summary - Post Wise</div>
...
const filteredRows = computed(() => {
    ...
    return rows.value.filter(row =>
        row.designation?.toLowerCase().includes(filter.value.toLowerCase())
    )
})
...
onMounted(async () => {
    const response = await axios.get(route('remuneration.json-summary-post-wise'))
...
const exportData = () => {
    ...
    axios.get(route('export.remuneration-summary-post-wise'), { responseType: 'blob' })
        .then((res) => {
            const fileUrl = window.URL.createObjectURL(new Blob([res.data]));
            const link = document.createElement('a');
            const currentYear = new Date().getFullYear(); // Get current year
            link.href = fileUrl;
            link.setAttribute('download', `remuneration-summary-post-wise-${currentYear}.xlsx`); // Use current year in filename
            link.click();
        })
        .catch((err) => {
            q.notify({
