<template>
    <q-page class="container" padding>
        ...
        <div class="flex items-center justify-between q-pa-md bg-white">
            <div>
                <div class="stitle">Work Charge Employee List: <q-chip square v-for="office in offices" :key="office.value" :label="office.label"/></div>
            ...
            <div class="q-gutter-sm">
                <q-btn label="WC Summary" color="primary" @click="$inertia.get(route('summary.office-wc'))" />
            </div>
        </div>
        <br/>
...
function onRequest (prop) {
    ...
    loading.value = true
    axios.get(route('employees.json-manager-wc'),{
        params:{
            filter,
            page,
            rowsPerPage,
...
const exportData = () => {
    ...
    axios.get(
        route('export.wc', firstOffice?.value),   // <-- pass ID only
        { responseType: 'blob' }
    )
        .then((res) => {
            ...
            link.setAttribute(
                'download',
                (firstOffice?.label || 'employees').replace(/\s+/g, '_') + '_wc_employees.xlsx'
            );
            link.click();
        })
        .catch((err) => {
